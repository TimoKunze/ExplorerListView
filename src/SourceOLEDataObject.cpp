...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerExLvw) {
			properties.pOwnerExLvw->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		if(secondChance) {
			if(properties.pOwnerExLvw->dragDropStatus.IsDragging()) {
				return S_OK;
			} else {
				// format not supported
				return DV_E_FORMATETC;
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerExLvw) {
				properties.pOwnerExLvw->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
void SourceOLEDataObject::SetOwner(ExplorerListView* pOwner)
{
	if(properties.pOwnerExLvw) {
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerExLvw) {
				properties.pOwnerExLvw->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
