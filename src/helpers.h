	...
	DoFinalProcessing: \
				...
				if(!wasHandled) { \
					if(hWnd == containedEdit) { \
						...
						wasHandled = TRUE; \
					} else if(hWnd == containedSysHeader32) { \
						lResult = containedSysHeader32.DefWindowProc(message, wParam, lParam); \
						wasHandled = TRUE; \
					} else if(hWnd == *this) { \
...
/// \brief <em>Used to retrieve or set a column's \c lParam data as it is stored by \c SysHeader32</em>
///
/// We give each column an unique ID, which is stored in the \c lParam member of its \c HDITEM
/// structure. The user-defined \c ColumnData value is stored in an internal hash map. To make the
/// whole thing 100% transparent, we intercept \c HDM_GETITEM and \c HDM_SETITEM messages and return
/// (modify) the value out of (in) the hash map if \c HDI_LPARAM is set. However, this prevents us
/// from retrieving (setting) a column's unique ID, so we introduce a custom \c HDI_* flag. If this
/// one is set, the \c ExplorerListView::OnHeaderGetItem (\c ExplorerListView::OnHeaderSetItem) method
/// will remove it and forward the message right to \c SysHeader32.
///
/// \sa ListViewColumn::get_ID, ListViewColumn::get_ColumnData, ExplorerListView::OnHeaderSetItem,
///     ExplorerListView::OnHeaderSetItem
#define HDI_NOINTERCEPTION 0x40000000
/// \brief <em>Used to retrieve or set an item's \c lParam data as it is stored by \c SysListView32</em>
///
/// We give each item an unique ID, which is stored in the \c lParam member of its \c LVITEM
/// structure. The user-defined \c ItemData value is stored in an internal hash map. To make the
/// whole thing 100% transparent, we intercept \c LVM_GETITEM and \c LVM_SETITEM messages and return
/// (modify) the value out of (in) the hash map if \c LVIF_PARAM is set. However, this prevents us
/// from retrieving (setting) an item's unique ID, so we introduce a custom \c LVIF_* flag. If this
/// one is set, the \c ExplorerListView::OnGetItem (\c ExplorerListView::OnSetItem) method
/// will remove it and forward the message right to \c SysListView32.
///
/// \sa ListViewItem::get_ID, ListViewItem::get_ItemData, ExplorerListView::OnGetItem,
///     ExplorerListView::OnSetItem
#define LVIF_NOINTERCEPTION 0x40000000
#ifdef INCLUDESHELLBROWSERINTERFACE
	/// \brief <em>Used to tell \c OnHeaderInsertItem to notify the attached \c ShellListView of the inserted column before raising the \c InsertedColumn event</em>
	///
	/// The \c IListViewColumn::ShellListViewColumnObject property requires \c ShellListView to have the
	/// unique IDs of all shell listview columns it is managing. When a new shell listview column is inserted
	/// into the listview control, \c ExplorerListView raises the \c InsertedColumn event before
	/// \c ShellListView gets the column's unique ID, so the \c IListViewItem::ShellListViewColumnObject
	/// property won't work during this event.\n
	/// We're solving this problem with a custom \c LVCF_* flag which tells
	/// \c ExplorerListView::OnHeaderInsertItem to tell \c ShellListView the inserted column's unique ID
	/// before raising the \c InsertedColumn event.
	///
	/// \sa ListViewItem::get_ShellListViewItemObject, ExplorerListView::OnHeaderInsertItem
	#define LVCF_NOTIFYSHELLLISTVIEW 0x80000000
	/// \brief <em>Used to tell \c OnInsertItem that the item being inserted is a shell item</em>
	///
	/// \c ShellListView needs to know whether the list view item being inserted is a shell item or a normal
	/// one in order to support the non-shell item image list. Therefore we use a custom \c LVIF_* flag to
	/// tell \c ShellListView the item's type.
	///
	/// \sa ExplorerListView::OnInsertItem
	#define LVIF_ISASHELLITEM 0x80000000
#endif

/// \brief <em>Listview columns' maximum caption length</em>
///
/// This is the maximum length (in characters) of a listview column's caption.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewColumn::get_Caption
#define MAX_COLUMNCAPTIONLENGTH MAX_PATH
...
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewItem::get_Text
#define MAX_ITEMTEXTLENGTH MAX_PATH
/// \brief <em>Listview footer items' maximum text length</em>
///
/// This is the maximum length (in characters) of a listview footer items's text.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewFooterItem::get_Text
#define MAX_FOOTERITEMTEXTLENGTH 4096
/// \brief <em>Listview groups' maximum subset link text length</em>
///
/// This is the maximum length (in characters) of the text of a listview group's subset link.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewGroup::get_SubsetLinkText
#define MAX_GROUPSUBSETLINKTEXTLENGTH 4096
/// \brief <em>Listview groups' maximum subtitle text length</em>
///
/// This is the maximum length (in characters) of a listview group's subtitle text.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewGroup::get_SubtitleText
#define MAX_GROUPSUBTITLETEXTLENGTH 4096
/// \brief <em>Listview groups' maximum task text length</em>
///
/// This is the maximum length (in characters) of a listview group's task text.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewGroup::get_TaskText
#define MAX_GROUPTASKTEXTLENGTH 4096
/// \brief <em>Listview groups' maximum description text length</em>
///
/// This is the maximum length (in characters) of a listview group's description text.
///
/// \remarks If we change this value, we also need to update the Doxygen comments in ExLvw*.idl!
///
/// \sa ListViewGroup::get_DescriptionTextBottom, ListViewGroup::get_DescriptionTextTop
#define MAX_GROUPDESCRIPTIONTEXTLENGTH 4096
...
/// \brief <em>Holds the parameters of a \c LVM_SORTGROUPS message</em>
///
/// To keep track of the positions of the listview's groups, we intercept \c LVM_SORTGROUPS and replace
/// the specified comparison function with our own before forwarding this message to \c SysListView32,
/// so we receive the original comparison function's return values which we use to synchronize our
/// internal data structures.\n
/// This struct holds the original parameters of the \c LVM_SORTGROUPS message, so we can call the
/// original comparison function like it would have been called without message interception.
///
/// \sa ExplorerListView::OnSortGroups, SortItemsDetails, SortItemsDetailsEx
typedef struct SortGroupsDetails
{
	/// \brief The original comparison function that was passed with the \c LVM_SORTGROUPS message
	PFNLVGROUPCOMPARE pRealComparisonFunction;
	/// \brief The original application-defined information that was passed with the \c LVM_SORTGROUPS message
	LPVOID pAppDefinedInformation;
	#ifdef USE_STL
		/// \brief The vector holding the groups' positions, that must be synchronized
		std::vector<int>* pGroups;
	#else
		/// \brief The vector holding the groups' positions, that must be synchronized
		CAtlArray<int>* pGroups;
	#endif
} SortGroupsDetails;

/// \brief <em>Holds the parameters of a \c LVM_SORTITEMS message</em>
///
/// To keep the indexes and IDs of the listview's items synchronized, we intercept \c LVM_SORTITEMS and
/// \c LVM_SORTITEMSEX and replace the specified comparison function with our own before forwarding this
/// message to \c SysListView32, so we receive the original comparison function's return values which we
/// use to synchronize our internal data structures.\n
/// This struct holds the original parameters of the \c LVM_SORTITEMS message, so we can call the
/// original comparison function like it would have been called without message interception.
///
/// \sa ExplorerListView::OnSortItems, SortItemsDetailsEx
typedef struct SortItemsDetails
{
	/// \brief The original comparison function that was passed with the \c LVM_SORTITEMS message
	PFNLVCOMPARE pRealComparisonFunction;
	/// \brief The original application-defined information that was passed with the \c LVM_SORTITEMS message
	LPARAM pAppDefinedInformation;
	#ifdef USE_STL
		/// \brief Holds a vector, that maps the new item indexes to their item IDs
		std::vector<LONG>* pNewItemIDs;
	#else
		/// \brief Holds an array, that maps the new item indexes to their item IDs
		CAtlArray<LONG>* pNewItemIDs;
	#endif
} SortItemsDetails;

/// \brief <em>Holds the parameters of a \c LVM_SORTITEMSEX message</em>
///
/// To keep the indexes and IDs of the listview's items synchronized, we intercept \c LVM_SORTITEMS and
/// \c LVM_SORTITEMSEX and replace the specified comparison function with our own before forwarding this
/// message to \c SysListView32, so we receive the original comparison function's return values which we
/// use to synchronize our internal data structures.\n
/// This struct holds the original parameters of the \c LVM_SORTITEMSEX message, so we can call the
/// original comparison function like it would have been called without message interception.
///
/// \sa ExplorerListView::OnSortItemsEx, SortItemsDetails
typedef struct SortItemsDetailsEx
{
	/// \brief The original comparison function that was passed with the \c LVM_SORTITEMSEX message
	PFNLVCOMPARE pRealComparisonFunction;
	/// \brief The original application-defined information that was passed with the \c LVM_SORTITEMSEX message
	LPARAM pAppDefinedInformation;
	#ifdef USE_STL
		/// \brief Holds a hash map, that maps the old item indexes to the new item indexes
		std::unordered_map<	int, int, ItemIndexHasher<int> >* pNewItemIndexes;
	#else
		/// \brief Holds a map, that maps the old item indexes to the new item indexes
		CAtlMap<int, int>* pNewItemIndexes;
	#endif
} SortItemsDetailsEx;
...
/// \brief <em>Retrieves whether a column index is a valid one</em>
///
/// \param[in] columnIndex The column index to check.
/// \param[in] pExLvw The \c ExplorerListView object containing the column.
///
/// \return \c TRUE if the column index is valid; otherwise \c FALSE.
...
///
/// \param[in] columnIndex The column index to check.
/// \param[in] hWndHeader The header control containing the column.
///
/// \return \c TRUE if the column index is valid; otherwise \c FALSE.
BOOL IsValidColumnIndex(int columnIndex, HWND hWndHeader);
/// \brief <em>Retrieves whether a footer item index is a valid one</em>
///
/// \param[in] footerItemIndex The footer item index to check.
/// \param[in] pExLvw The \c ExplorerListView object containing the footer item.
///
/// \return \c TRUE if the footer item index is valid; otherwise \c FALSE.
...
///
/// \param[in] footerItemIndex The footer item index to check.
/// \param[in] hWndLvw The listview control containing the footer item.
///
/// \return \c TRUE if the footer item index is valid; otherwise \c FALSE.
BOOL IsValidFooterItemIndex(int footerItemIndex, HWND hWndLvw);
/// \brief <em>Retrieves whether a group ID is a valid one</em>
///
/// \param[in] groupID The group ID to check.
/// \param[in] pExLvw The \c ExplorerListView object containing the group.
///
/// \return \c TRUE if the group ID is valid; otherwise \c FALSE.
...
///
/// \param[in] groupID The group ID to check.
/// \param[in] hWndLvw The listview control containing the group.
///
/// \return \c TRUE if the group ID is valid; otherwise \c FALSE.
BOOL IsValidGroupID(int groupID, HWND hWndLvw);
/// \brief <em>Retrieves whether an item index is a valid one</em>
///
/// \param[in] itemIndex The item index to check.
/// \param[in] pExLvw The \c ExplorerListView object containing the item.
///
...
///
/// \param[in] itemIndex The item index to check.
/// \param[in] hWndLvw The listview control containing the item.
///
/// \return \c TRUE if the item index is valid; otherwise \c FALSE.
BOOL IsValidItemIndex(int itemIndex, HWND hWndLvw);
/// \brief <em>Retrieves whether a working area index is a valid one</em>
///
/// \param[in] workAreaIndex The working area index to check.
/// \param[in] pExLvw The \c ExplorerListView object containing the working area.
///
/// \return \c TRUE if the working area index is valid; otherwise \c FALSE.
...
///
/// \param[in] workAreaIndex The working area index to check.
/// \param[in] hWndLvw The listview control containing the working area.
///
/// \return \c TRUE if the working area index is valid; otherwise \c FALSE.
BOOL IsValidWorkAreaIndex(int workAreaIndex, HWND hWndLvw);

/// \brief <em>A callback function for group sorting</em>
///
/// To keep track of the positions of the listview's groups, we intercept \c LVM_SORTGROUPS and replace
/// the specified comparison function with this one before forwarding this message to \c SysListView32,
/// so we receive the original comparison function's return values which we use to synchronize our internal
/// data structures.
///
/// \sa ExplorerListView::OnSortGroups, MyLVCompareFunc, MyLVCompareFuncEx,
///     <a href="https://msdn.microsoft.com/en-us/library/ms671390.aspx">LVGroupCompare</a>
INT CALLBACK MyLVGroupCompare(INT Group1_ID, INT Group2_ID, LPVOID pvData);
/// \brief <em>A callback function for item sorting</em>
///
/// To keep the indexes and IDs of the listview's items synchronized, we intercept \c LVM_SORTITEMS and
/// \c LVM_SORTITEMSEX and replace the specified comparison function with our own before forwarding this
/// message to \c SysListView32, so we receive the original comparison function's return values which we
/// use to synchronize our internal data structures.
///
/// \sa ExplorerListView::OnSortItems, MyLVCompareFunc, MyQuickSortCompareFuncEx,
///     <a href="https://msdn.microsoft.com/en-us/library/4xc60xas.aspx">qsort_s</a>
...
/// use to synchronize our internal data structures.
///
/// \sa ExplorerListView::OnSortItems, MyQuickSortCompareFunc, MyLVCompareFuncEx, MyLVGroupCompare,
...
/// use to synchronize our internal data structures.
///
/// \sa ExplorerListView::OnSortItemsEx, MyLVCompareFuncEx, MyQuickSortCompareFunc,
///     <a href="https://msdn.microsoft.com/en-us/library/4xc60xas.aspx">qsort_s</a>
...
/// use to synchronize our internal data structures.
///
/// \sa ExplorerListView::OnSortItemsEx, MyQuickSortCompareFuncEx, MyLVCompareFunc, MyLVGroupCompare,
...
///
/// \sa ExplorerListView::OnSetBkImage, TargetOLEDataObject
...
///
/// This function compares two listview groups. Its the callback function that we defined when sending
/// the \c LVM_SORTGROUPS message to the control.
///
...
/// \param[in] pvData Additional data that was passed to the control along with the \c LVM_SORTGROUPS
///            message. Usually this is a pointer to the \c IGroupComparator implementation of the
///            object that initiated the sorting.
///
...
///
/// \sa ExplorerListView::SortGroups, IGroupComparator::CompareGroups
...
/// the \c LVM_SORTITEMS message to the control.
///
...
///            message. Usually this is a pointer to the \c IItemComparator implementation of the object
...
///
/// \sa CompareItemsEx, ExplorerListView::SortItems, IItemComparator::CompareItems
int CALLBACK CompareItems(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
/// \brief <em>Two items must be compared</em>
///
/// This function compares two listview items. Its the callback function that we defined when sending
/// the \c LVM_SORTITEMSEX message to the control.
///
/// \param[in] lParam1 The index of the first item to compare.
/// \param[in] lParam2 The index of the second item to compare.
/// \param[in] lParamSort Additional data that was passed to the control along with the \c LVM_SORTITEMSEX
///            message. Usually this is a pointer to the \c IItemComparator implementation of the object
///            that initiated the sorting.
///
/// \return -1 if the first item should precede the second; 0 if the items are equal; 1 if the
///         second item should precede the first.
///
/// \sa CompareItems, ExplorerListView::SortItems, IItemComparator::CompareItemsEx
...
///
/// \sa ExplorerListView::OnGetDragImage, ExplorerListView::OnHeaderGetDragImage,
///     ExplorerListView::OLEDrag, ExplorerListView::HeaderOLEDrag,
...
/// required for \c LVM_GETGROUPINFO which is poorly implemented on Windows Vista.
///
...
///
/// \sa ListViewGroup::get_SubtitleText, ListViewGroup::get_TaskText
